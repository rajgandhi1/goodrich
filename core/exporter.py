...
def build_excel(items: list[dict], customer: str = '', project_ref: str = '') -> bytes:
    ...
    # --- Header rows ---
    ws.merge_range('A1:J1', f'GOODRICH GASKET PVT. LTD', wb.add_format(
        {'bold': True, 'font_size': 14, 'align': 'center', 'valign': 'vcenter'}))
    if customer or project_ref:
        ws.merge_range('A2:J2', f'Customer: {customer}   |   Ref: {project_ref}',
    ...
    headers = ['SR.NO', 'CUSTOMER DESCRIPTION', 'GGPL DESCRIPTION',
               ...
               'QTY', 'UOM', 'OD (MM)', 'ID (MM)', 'STATUS', 'FLAGS / DEFAULTS']
    col_widths = [6, 40, 50, 8, 8, 25, 5, 8, 6, 5, 8, 8, 10, 40]
    ...
    for i, item in enumerate(items, 1):
        ...
        ws.write(row, 10, dims.get('od', ''),                   row_fmt)
        ws.write(row, 11, dims.get('id', ''),                   row_fmt)
        ws.write(row, 12, status.upper(),                       row_fmt)
        ws.write(row, 13, notes,                                row_fmt)
        ws.set_row(row, 30)
