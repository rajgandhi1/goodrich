...
def build_excel(items: list[dict], customer: str = '', project_ref: str = '') -> bytes:
    ...
    for i, item in enumerate(items, 1):
        ...
        ws.write(row, 7,  item.get('series', '') or '',                row_fmt)
        ws.write(row, 8,  item.get('thickness_mm', ''),                row_fmt)
        ws.write(row, 9,  item.get('quantity', ''),                    row_fmt)
        ws.write(row, 10, item.get('uom', ''),                         row_fmt)
        ws.write(row, 11, dims.get('od', '') if dims else '',          row_fmt)
        ws.write(row, 12, dims.get('id', '') if dims else '',          row_fmt)
        ws.write(row, 13, item.get('ring_no', ''),                     row_fmt)
        ws.write(row, 14, item.get('rtj_groove_type', ''),             row_fmt)
        hardness_display = item.get('rtj_hardness_spec') or item.get('rtj_hardness_bhn', '')
        ws.write(row, 15, hardness_display,                             row_fmt)
        ws.write(row, 16, item.get('sw_winding_material', ''),         row_fmt)
        ws.write(row, 17, item.get('sw_filler', ''),                   row_fmt)
        ws.write(row, 18, item.get('sw_outer_ring', ''),               row_fmt)
        ws.write(row, 19, item.get('sw_inner_ring', ''),               row_fmt)
        ws.write(row, 20, status.upper(),                              row_fmt)
        ws.write(row, 21, notes,                                       row_fmt)
        ws.set_row(row, 30)
